#!/usr/bin/env python3
"""
Ball tracking v7c — v7b + gap-consistency filter + trajectory-guided selection.

New vs v7b:
  1. Trajectory-guided selection: for each frame, pick ball candidate CLOSEST
     to running trajectory estimate (not highest confidence). Falls back to
     highest confidence if it's above hard_reset_conf and nothing close found.
     This prevents "slowly drifting false positive chain" since the actual ball
     (closer to last known position) wins over a drifting false positive.

  2. Gap consistency filter: after initial collection + ABC filter, apply a
     "band filter" for each gap between established anchors:
     - Compute linear interpolation between anchor A and anchor B
     - Remove any detection in the gap that is > gap_margin from the interp line
     - This catches chains of false positives that drift away from true trajectory
...
def collect_trajectory_guided(cache, n_frames, seed_pos,
                               ...
                               vel_decay=0.85):
    """
    Sequential collection: maintain running estimate, pick candidate
    closest to running estimate (within speed_limit per frame).
    If nothing close but a high-confidence detection exists, hard-reset.
    """
    rx, ry = float(seed_pos[0]), float(seed_pos[1])
    rvx, rvy = 0.0, 0.0  # velocity estimate
    last_det_frame = 0
    dets = {}
    ...
    for idx in range(n_frames):
        ...
        if close:
            ...
            # Hard reset: pick highest-confidence far detection
...
# ─────────────────────────────────────────────────────────────
# Pass 1c: Gap consistency filter
# ─────────────────────────────────────────────────────────────

def gap_consistency_filter(clean_dets, seed_frame, seed_pos,
                            gap_margin=120.0,
                            min_gap_for_filter=15,
                            hard_reset_conf=0.40):
    """
    For each gap between established anchors, remove detections that deviate
    more than gap_margin from the linear interpolation between anchors.

    This catches slowly-drifting false positive chains that the speed filter
    and ABC filter miss (each step is small, but the chain drifts far).
    """
    filtered = dict(clean_dets)
    # Add seed as fixed anchor
    anchors = dict(filtered)
        ...
        for i in range(len(anchor_frames)-1):
            ...
            for f in list(filtered.keys()):
                ...
                if cf >= hard_reset_conf:
                    continue
                # Expected position via linear interp
                t = (f - f_a) / gap
                ex = ax + t * (bx - ax)
                ey = ay + t * (by - ay)
...
def track_v7c(frames, seed_frame, seed_pos, cache,
              min_conf=0.10,
              hard_reset_conf=0.40,
              speed_limit=250.0,
              vel_decay=0.85,
    ...
    # Pass 1c: Gap consistency filter
    clean_dets = gap_consistency_filter(
    ...
    # Pass 3: optional Kalman smoothing
...
def main():
    ...
    # Baseline
    print("\n--- Baseline v7c ---")
    pos = track_v7c(frames, seed_frame, seed_pos, cache)
    ...
    hand_tuned = [
        dict(min_conf=0.10, hard_reset_conf=0.40, speed_limit=250, vel_decay=0.85,
             abc_ratio=0.6, gap_margin=120, min_gap_for_filter=15,
        ...
        dict(min_conf=0.10, hard_reset_conf=0.40, speed_limit=200, vel_decay=0.85,
             abc_ratio=0.5, gap_margin=100, min_gap_for_filter=10,
        ...
        dict(min_conf=0.08, hard_reset_conf=0.35, speed_limit=300, vel_decay=0.80,
             abc_ratio=0.7, gap_margin=150, min_gap_for_filter=20,
             interp_max_gap=60, y_off=48, x_off=0, person_accept_r=180,
             person_conf_min=0.25, use_kalman_smooth=True, kalman_q=1.5),
        dict(min_conf=0.15, hard_reset_conf=0.50, speed_limit=180, vel_decay=0.90,
             abc_ratio=0.55, gap_margin=80, min_gap_for_filter=12,
        ...
        dict(min_conf=0.30, hard_reset_conf=0.50, speed_limit=150, vel_decay=0.85,
             abc_ratio=0.5, gap_margin=120, min_gap_for_filter=15,
             interp_max_gap=120, y_off=58, x_off=0, person_accept_r=100,
             person_conf_min=0.30, use_kalman_smooth=True, kalman_q=2.0),
        dict(min_conf=0.10, hard_reset_conf=0.40, speed_limit=250, vel_decay=0.85,
             abc_ratio=0.4, gap_margin=150, min_gap_for_filter=10,
             interp_max_gap=120, y_off=51, x_off=0, person_accept_r=150,
             person_conf_min=0.30, use_kalman_smooth=False, kalman_q=1.0),
        dict(min_conf=0.12, hard_reset_conf=0.45, speed_limit=200, vel_decay=0.88,
             abc_ratio=0.6, gap_margin=100, min_gap_for_filter=15,
             interp_max_gap=80, y_off=54, x_off=0, person_accept_r=100,
             person_conf_min=0.35, use_kalman_smooth=True, kalman_q=1.0),
    ]
    ...
    for _ in range(72):
        random_combos.append(dict(
            ...
            gap_margin      = random.choice([60, 80, 100, 120, 150, 200]),
    ...
    for ci, params in enumerate(all_combos):
        pos = track_v7c(frames, seed_frame, seed_pos, cache, **params)
        tr, w30, w50, med, sc = evaluate(pos, labels)
        tag = (f"mc={params['min_conf']:.2f} sl={params['speed_limit']:.0f} "
               f"abc={params['abc_ratio']:.1f} gm={params['gap_margin']:.0f} "
    ...
    with open('tracking_analysis_v7c.txt', 'w') as f:
        f.write(f"V7c Winner: score={sc:.4f} w30={w30} w50={w50} med={med:.1f}\n")
        f.write(f"Params: {best_params}\n\n")
        for s, w3, w5, md, t, tg, _, _ in results_log[:20]:
            f.write(f"{s:.4f}  W30={w3}  W50={w5}  Med={md:.1f}  {tg}\n")
    print("\nSaved to tracking_analysis_v7c.txt")

    if args.render:
        render_video(frames, best_pos, labels, 'tracking_v7c_winner.mp4')

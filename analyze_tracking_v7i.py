#!/usr/bin/env python3
"""
Ball tracking v7g — v7f + split abc_protect_conf from hard_reset_conf.

Key insight from v7f analysis:
  hc_override adds f265@(1269,494) c=0.51 and f1020@(1554,496) c=0.63 since
  both exceed hrc=0.50. abc_filter protects them (bcf >= hard_reset_conf=0.50)
  so they can't be removed. This lets FPs in the 0.50-0.64 conf range through.

Fix: separate abc_protect_conf (default 0.65) from hard_reset_conf (0.50).
  - hc_override still uses hard_reset_conf=0.50 to include f940@c=0.56, etc.
  - abc_filter uses abc_protect_conf=0.65 → only confs ≥0.65 are "safe"
  - f265@c=0.51 becomes LC for abc_filter → removed (91px < 0.6*168px ✓)
  - f986@c=0.56 becomes LC → removed (dist pattern matches reversal)
  - f1020@c=0.63 becomes LC → abc may remove, or validate_hc_chain catches it
  - f295@c=0.70, f379@c=0.76, f941@c=0.68 → still protected ✓

Also: chain_margin=350 remains to allow f295 (dev=319px < 350).
...
def abc_filter(dets, abc_protect_conf=0.65, speed_limit=250.0, abc_ratio=0.6):
    """A→B→C consistency filter. Only removes detections with conf < abc_protect_conf."""
    clean = dict(dets)
    for _ in range(3):
        frames = sorted(clean)
        to_remove = set()
...
# ─────────────────────────────────────────────────────────────
# Pass 1f: Two-stage LC inclusion
# ─────────────────────────────────────────────────────────────

def lc_inclusion(validated_dets, cache, n_frames, seed_frame, seed_pos,
                 lc_min_conf=0.05,
                 lc_max_conf=0.12,
                 lc_margin=200.0,
                 lc_min_gap=5):
    """
    After HC anchor validation, include very-low-confidence detections that
    are close to the validated trajectory interpolation between anchors.

    This captures correct but below-threshold detections like:
      f310@(497,474) c=0.10 (2px from GT, dev=243px from f303→f379 line)
      f945@(759,515) c=0.07 (1px from GT)
      f1000@(713,475) c=0.06 (3px from GT)

    Only adds detections with lc_min_conf <= conf < lc_max_conf that are
    within lc_margin px of the interpolated anchor line.
    """
    anchors = dict(validated_dets)
    ...
    for i in range(len(anchor_frames) - 1):
        ...
        for idx in range(fa + 1, fb):
            if idx in validated_dets:
                continue  # already have a detection here
            balls = cache.get(idx, {}).get('balls', [])
            candidates = [b for b in balls if lc_min_conf <= b[2] < lc_max_conf]
            if not candidates:
                continue
            ...
            # Pick the candidate closest to the interpolated position
            best = min(candidates, key=lambda b: dist((b[0], b[1]), (ex, ey)))
            if dist((best[0], best[1]), (ex, ey)) <= lc_margin:
                extra[idx] = (best[0], best[1], best[2])

    merged = dict(validated_dets)
    merged.update(extra)
    return merged
...
def track_v7i(frames, seed_frame, seed_pos, cache,
              min_conf=0.12,
              ...
              lc_margin=200.0,        # NEW: max dev from anchor line for LC inclusion
              lc_max_conf=0.12,       # NEW: include detections with conf < this in LC pass
              interp_max_gap=100,
              y_off=47.0,
              x_off=0.0,
              person_accept_r=150.0,
    ...
    # NEW: two-stage LC inclusion — add very-low-conf detections near validated trajectory
    clean_dets = lc_inclusion(
        clean_dets, cache, n, seed_frame, seed_pos,
        lc_min_conf=0.05,
        lc_max_conf=lc_max_conf,
        lc_margin=lc_margin,
        lc_min_gap=5)
...
def main():
    ...
    print("\n--- Baseline v7i ---")
    pos = track_v7i(frames, seed_frame, seed_pos, cache)
    ...
    hand_tuned = [
        # v7g winner + hc_thresh lowered to 0.55 → f303@0.64 becomes HC anchor
        # This allows f310@(497) (dev=242px from f303→f377) to pass gap filter
        # And chain_margin=400 still removes f1020@0.63 (dev=522px > 400)
        dict(min_conf=0.12, hard_reset_conf=0.50, abc_protect_conf=0.65,
             speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.55, chain_margin=400, gap_margin=320,
        ...
        # Same with gm=260 (tighter gap filter)
        dict(min_conf=0.12, hard_reset_conf=0.50, abc_protect_conf=0.65,
             speed_limit=120, vel_decay=0.85,
             ...
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # hc_thresh=0.55, cm=350 (f1020 dev=522>350, f295 dev=301<350)
        dict(min_conf=0.12, hard_reset_conf=0.50, abc_protect_conf=0.65,
             speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.55, chain_margin=350, gap_margin=320,
             min_gap_for_filter=6, interp_max_gap=100, y_off=47, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # hc_thresh=0.55, yo=65, gm=260
        dict(min_conf=0.05, hard_reset_conf=0.50, abc_protect_conf=0.65,
             speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.55, chain_margin=400, gap_margin=260,
             min_gap_for_filter=8, interp_max_gap=120, y_off=65, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # abc_protect=0.70 with hc_thresh=0.55
        dict(min_conf=0.12, hard_reset_conf=0.50, abc_protect_conf=0.70,
             speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.55, chain_margin=400, gap_margin=320,
             min_gap_for_filter=6, interp_max_gap=100, y_off=47, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # No abc_protect change (revert to 0.50 protection) with hc_thresh=0.55
        dict(min_conf=0.12, hard_reset_conf=0.50, abc_protect_conf=0.50,
             speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.55, chain_margin=400, gap_margin=320,
             min_gap_for_filter=6, interp_max_gap=100, y_off=47, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # v7g winner unchanged (baseline comparison)
        dict(min_conf=0.12, hard_reset_conf=0.50, abc_protect_conf=0.65,
             speed_limit=120, vel_decay=0.85,
             ...
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # hc_thresh=0.55, yo=58, gm=280
        dict(min_conf=0.10, hard_reset_conf=0.50, abc_protect_conf=0.65,
             speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.55, chain_margin=400, gap_margin=280,
             min_gap_for_filter=8, interp_max_gap=120, y_off=58, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # hc_thresh=0.55, mgf=8, gm=280
        dict(min_conf=0.12, hard_reset_conf=0.50, abc_protect_conf=0.65,
             speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.55, chain_margin=400, gap_margin=280,
             min_gap_for_filter=8, interp_max_gap=100, y_off=47, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # hc_thresh=0.60 (intermediate)
        dict(min_conf=0.12, hard_reset_conf=0.50, abc_protect_conf=0.65,
             speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.60, chain_margin=400, gap_margin=320,
             min_gap_for_filter=6, interp_max_gap=100, y_off=47, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
    ]
    ...
    for _ in range(90):
        random_combos.append(dict(
            ...
            abc_protect_conf  = random.choice([0.50, 0.55, 0.60, 0.65, 0.70, 0.75]),
            speed_limit       = random.choice([80, 100, 120, 150, 200]),
            vel_decay         = random.choice([0.75, 0.80, 0.85, 0.90, 0.95]),
            abc_ratio         = random.choice([0.4, 0.5, 0.55, 0.6, 0.65, 0.7]),
            hc_thresh         = random.choice([0.50, 0.55, 0.60, 0.65]),
            chain_margin      = random.choice([300, 350, 400, 450, 500]),
            gap_margin        = random.choice([220, 240, 260, 280, 300, 320, 360]),
    ...
    for ci, params in enumerate(all_combos):
        pos = track_v7i(frames, seed_frame, seed_pos, cache, **params)
        tr, w30, w50, med, sc = evaluate(pos, labels)
        tag = (f"mc={params['min_conf']:.2f} hrc={params['hard_reset_conf']:.2f} "
               f"apc={params['abc_protect_conf']:.2f} "
    ...
    with open('tracking_analysis_v7i.txt', 'w') as f:
    ...
    print("\nSaved to tracking_analysis_v7i.txt")

    if args.render:
        render_video(frames, best_pos, labels, 'tracking_v7i_winner.mp4')

...
def track_v7h(frames, seed_frame, seed_pos, cache,
              min_conf=0.10,
              hard_reset_conf=0.50,   # used in trajectory collection + hc_override
              abc_protect_conf=0.65,  # NEW: abc_filter only removes conf < this
              speed_limit=120.0,
              vel_decay=0.85,
              abc_ratio=0.6,
              hc_thresh=0.55,         # threshold for HC anchors (chain + gap filter)
              chain_margin=350.0,     # max dev for HC anchor chain validation
    ...
    # Include any HC detection that trajectory-guided missed
    raw_dets = hc_override(raw_dets, cache, n, hc_direct_conf=hard_reset_conf)

    # abc_filter: protect only very high-conf detections (abc_protect_conf)
    # so FPs in the 0.50-0.64 conf range (e.g. f265@0.51, f1020@0.63) can be removed
...
def main():
    ...
    print("\n--- Baseline v7h ---")
    pos = track_v7h(frames, seed_frame, seed_pos, cache)
    ...
    for ci, params in enumerate(all_combos):
        pos = track_v7h(frames, seed_frame, seed_pos, cache, **params)
        tr, w30, w50, med, sc = evaluate(pos, labels)
    ...
    with open('tracking_analysis_v7h.txt', 'w') as f:
    ...
    print("\nSaved to tracking_analysis_v7h.txt")

    if args.render:
        render_video(frames, best_pos, labels, 'tracking_v7h_winner.mp4')

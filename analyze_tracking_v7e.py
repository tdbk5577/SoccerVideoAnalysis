#!/usr/bin/env python3
"""
Ball tracking v7e — v7d + HC anchor chain validation.

Key insight from v7d analysis:
  Three HC FPs corrupt large gaps:
    f335 (1501,466) c=0.52 — 994px from GT=(508,493)
    f340 (1522,456) c=0.54 — 995px from GT
    f342-f349 cluster (x~1500-1530) — same FP cluster
    f860  (85,672)  c=0.47 — 832px from GT=(903,519)
    f1020 (1555,496) c=0.63 — 743px from GT=(812,486)

  These FPs become HC anchors that corrupt the gap filter's
  interpolation baseline, causing correct LC detections (like
  f310@(497,475) c=0.10 which is 1px from GT) to be filtered out.

Fix: validate_hc_chain() — iteratively removes HC anchors that deviate
  most from the line between their neighboring HC anchors ("remove worst
  first"). This correctly removes the FP cluster before the FPs can
  mutually validate each other.

  Also: increase gap_margin to 300 so correct LC detections
  at f310 (243px from HC-HC line) survive the gap filter.
...
# ─────────────────────────────────────────────────────────────
# Pass 1c: HC anchor chain validation  ← NEW in v7e
# ─────────────────────────────────────────────────────────────

def validate_hc_chain(clean_dets, seed_frame, seed_pos,
                      hc_thresh=0.40, chain_margin=300.0):
    """
    Remove HC FPs by checking each HC anchor against its neighbors in the HC chain.

    Uses "remove worst first" iterative approach: always removes the HC anchor
    with the HIGHEST deviation from its (A→C) interpolated position. This avoids
    removing correct anchors that appear to deviate only because neighboring FPs
    are still present.

    Example: given correct f303@(763) and FPs f335@(1501), f340@(1522),
    checking f335 against f303→f377 gives dev=870px (highest) → remove f335 first.
    Then f340 against f303→f377 gives dev=912px → remove f340.
    Finally f303 against f295→f377 gives dev=281px < chain_margin=300 → kept.
    """
    ...
    while True:
        ...
        worst_frame = None
        worst_dev = chain_margin  # only remove if deviation > chain_margin
        ...
        for i in range(1, len(hc_frames)-1):
            ...
            # Interpolate from A to C at frame B
            t = (fb - fa) / (fc - fa)
            ex = ax + t * (cx - ax)
            ey = ay + t * (cy - ay)
        ...
        if worst_frame is None:
            break  # No more outliers above chain_margin
...
# ─────────────────────────────────────────────────────────────
# Pass 1d: HC-boundary gap consistency filter
# ─────────────────────────────────────────────────────────────
...
def hc_gap_filter(clean_dets, seed_frame, seed_pos,
    ...
    With gap_margin=300 (up from v7d's 120-160), correct LC detections that
    fall during fast-trajectory phases (e.g. f310@(497,475) is 243px from the
    f303→f377 line) now survive and serve as anchors for subsequent interpolation.
    """
    filtered = dict(clean_dets)
...
def track_v7e(frames, seed_frame, seed_pos, cache,
              min_conf=0.10,
              hard_reset_conf=0.40,
              speed_limit=250.0,
              vel_decay=0.85,
              abc_ratio=0.6,
              hc_thresh=0.40,
              chain_margin=300.0,   # NEW: HC chain validation margin
              gap_margin=300.0,     # Increased from v7d's 140
              min_gap_for_filter=15,
              interp_max_gap=80,
              y_off=51.0,
              x_off=0.0,
    ...
    # NEW: validate HC anchors before gap filter
...
def main():
    ...
    print("\n--- Baseline v7e ---")
    pos = track_v7e(frames, seed_frame, seed_pos, cache)
    ...
    hand_tuned = [
        # Targeted fix: HC chain validation + large gap_margin
        dict(min_conf=0.05, hard_reset_conf=0.40, speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.60, chain_margin=300, gap_margin=300,
             min_gap_for_filter=15, interp_max_gap=120, y_off=65, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # Tighter chain_margin
        dict(min_conf=0.05, hard_reset_conf=0.40, speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.55, chain_margin=250, gap_margin=280,
             min_gap_for_filter=15, interp_max_gap=120, y_off=65, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # Lower hc_thresh
        dict(min_conf=0.08, hard_reset_conf=0.40, speed_limit=150, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.40, chain_margin=280, gap_margin=260,
             min_gap_for_filter=15, interp_max_gap=100, y_off=58, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # Higher chain_margin (less aggressive removal)
        dict(min_conf=0.05, hard_reset_conf=0.40, speed_limit=120, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.60, chain_margin=400, gap_margin=350,
             min_gap_for_filter=15, interp_max_gap=120, y_off=65, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
        # No kalman
        ...
        # Variant with tighter speed limit and y_off=47
        dict(min_conf=0.05, hard_reset_conf=0.35, speed_limit=100, vel_decay=0.90,
             abc_ratio=0.7, hc_thresh=0.55, chain_margin=300, gap_margin=300,
             min_gap_for_filter=12, interp_max_gap=150, y_off=47, x_off=0,
             person_accept_r=180, person_conf_min=0.25,
             use_kalman_smooth=True, kalman_q=0.5),
        # Wide interp gap
        dict(min_conf=0.05, hard_reset_conf=0.50, speed_limit=200, vel_decay=0.85,
             abc_ratio=0.5, hc_thresh=0.65, chain_margin=350, gap_margin=350,
             min_gap_for_filter=20, interp_max_gap=200, y_off=65, x_off=0,
             person_accept_r=100, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=2.0),
        # Very tight chain, rely more on gap filter
        dict(min_conf=0.08, hard_reset_conf=0.40, speed_limit=150, vel_decay=0.85,
             abc_ratio=0.6, hc_thresh=0.50, chain_margin=200, gap_margin=350,
             min_gap_for_filter=10, interp_max_gap=80, y_off=58, x_off=0,
             person_accept_r=150, person_conf_min=0.30,
             use_kalman_smooth=True, kalman_q=1.0),
    ]
    ...
    for _ in range(92):
        random_combos.append(dict(
            min_conf        = random.choice([0.05, 0.08, 0.10, 0.12, 0.15, 0.20]),
            hard_reset_conf = random.choice([0.30, 0.35, 0.40, 0.50, 0.60]),
            speed_limit     = random.choice([100, 120, 150, 200, 250, 300]),
            vel_decay       = random.choice([0.75, 0.80, 0.85, 0.90, 0.95]),
            abc_ratio       = random.choice([0.4, 0.5, 0.6, 0.7]),
            hc_thresh       = random.choice([0.40, 0.45, 0.50, 0.55, 0.60, 0.65]),
            chain_margin    = random.choice([200, 250, 280, 300, 350, 400, 500]),
            gap_margin      = random.choice([200, 250, 280, 300, 350, 400]),
            min_gap_for_filter = random.choice([8, 10, 12, 15, 20]),
            interp_max_gap  = random.choice([60, 80, 100, 120, 150, 200]),
            y_off           = random.choice([44, 47, 51, 54, 58, 65]),
            x_off           = random.choice([-5, 0, 5]),
            person_accept_r = random.choice([80, 120, 150, 200]),
    ...
    for ci, params in enumerate(all_combos):
        pos = track_v7e(frames, seed_frame, seed_pos, cache, **params)
        tr, w30, w50, med, sc = evaluate(pos, labels)
        tag = (f"mc={params['min_conf']:.2f} sl={params['speed_limit']:.0f} "
               f"hc={params['hc_thresh']:.2f} cm={params['chain_margin']:.0f} "
               f"gm={params['gap_margin']:.0f} ig={params['interp_max_gap']} "
               f"yo={params['y_off']}")
    ...
    with open('tracking_analysis_v7e.txt', 'w') as f:
        f.write(f"V7e Winner: score={sc:.4f} w30={w30} w50={w50} med={med:.1f}\n")
        f.write(f"Params: {best_params}\n\n")
        for s, w3, w5, md, t, tg, _, _ in results_log[:20]:
            f.write(f"{s:.4f}  W30={w3}  W50={w5}  Med={md:.1f}  {tg}\n")
    print("\nSaved to tracking_analysis_v7e.txt")

    if args.render:
        render_video(frames, best_pos, labels, 'tracking_v7e_winner.mp4')
